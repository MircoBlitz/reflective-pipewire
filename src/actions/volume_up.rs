...
impl Default for VolumeUpSettings {
    fn default() -> Self {
        Self {
...
static SETTINGS: LazyLock<DashMap<InstanceId, VolumeUpSettings>> = LazyLock::new(DashMap::new);

pub struct VolumeUpAction;

#[async_trait]
impl Action for VolumeUpAction {
    const UUID: ActionUuid = "de.mircoblitz.reflective-pipewire.volume-up";
    type Settings = VolumeUpSettings;
...
pub async fn sync_all_instances() {
    for inst in visible_instances(VolumeUpAction::UUID).await {
...
async fn render_button(instance: &Instance, s: &VolumeUpSettings) -> OpenActionResult<()> {
